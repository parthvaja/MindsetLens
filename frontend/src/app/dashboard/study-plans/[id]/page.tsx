...
import {
  ...
  Printer, ArrowLeft, Sparkles, SendHorizonal, X,
} from 'lucide-react';
...
function CalloutCard({ callout, colorMap }: { callout: StudentCallout; colorMap: Record<string, typeof STUDENT_COLORS[0]> }) {
  ...
  return (
    <div className={cn('rounded-xl border p-3.5 space-y-1.5', colors.bg, colors.border)}>
...
function SectionBlock({ section, colorMap, defaultOpen = true }: { section: PlanSection; colorMap: Record<string, typeof STUDENT_COLORS[0]>; defaultOpen?: boolean }) {
  const [open, setOpen] = useState(defaultOpen);
  const meta = SECTION_META[section.id] ?? { emoji: '📌', accent: 'from-zinc-700/30 to-zinc-700/10 border-zinc-700/30' };

  return (
    <div className="bg-zinc-900 rounded-2xl border border-zinc-800 overflow-hidden print:break-inside-avoid">
      <button
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center gap-3.5 px-5 py-4 hover:bg-zinc-800/40 transition-colors text-left"
      >
        <div className={cn('w-9 h-9 rounded-xl bg-gradient-to-br border flex items-center justify-center text-lg shrink-0', meta.accent)}>
          <span role="img" aria-hidden>{meta.emoji}</span>
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-zinc-100">{section.title}</p>
          {section.duration && (
...
export default function StudyPlanDetailPage() {
  ...
  if (plan?.plan_content?.sections) {
    for (const section of plan.plan_content.sections) {
      ...
      for (const c of callouts) {
        if (!studentNames.includes(c.student_name)) studentNames.push(c.student_name);
      }
    }
  }
  ...
  if (isLoading) {
    return (
      <div className="space-y-4 animate-pulse">
        <div className="h-8 w-56 bg-zinc-800 rounded-xl" />
  ...
  const sections = plan.plan_content?.sections ?? [];
    ...
    >
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <button
            onClick={() => router.push('/dashboard/study-plans')}
            className="mt-0.5 p-1.5 rounded-lg text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300 transition-all"
            title="Back to Study Plans"
          >
            <ArrowLeft size={15} />
          </button>
          <div>
            <h1 className="text-[28px] font-heading font-semibold text-zinc-50 tracking-tight leading-tight">
              {plan.topic}
            </h1>
        ...
        <button
          onClick={handlePrint}
          className="shrink-0 flex items-center gap-2 px-3.5 py-2 rounded-xl border border-zinc-700 text-xs text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200 transition-all print:hidden"
        >
          <Printer size={13} />
          Print
        </button>
      ...
      {/* Context notes */}
      {plan.context_notes && (
        <div className="px-4 py-3 rounded-xl bg-zinc-800/60 border border-zinc-700/50 text-xs text-zinc-400">
